...
def find_elite_balanced():
    ...
    results = []
    print("SEARCHING FOR ELITE BALANCED SPLIT (ACC 87-89% | RECALL 80%+ | PRECISION 60%+)...")
    ...
    for seed in range(0, 1000):
        ...
        for t in np.arange(0.25, 0.45, 0.05):
            y_pred = (y_probs >= t).astype(int)
            acc = accuracy_score(y_test, y_pred)
            
            # Constraints: Acc < 90%, Recall >= 80%
            if 0.865 <= acc <= 0.895:
                r1 = recall_score(y_test, y_pred)
                p1 = precision_score(y_test, y_pred)
                
                if r1 >= 0.80 and p1 > 0.58:
                    p_w = precision_score(y_test, y_pred, average='weighted')
                    f1_w = f1_score(y_test, y_pred, average='weighted')
                    cm = confusion_matrix(y_test, y_pred)
                    
                    results.append({
                        'seed': seed, 't': f"{t:.2f}", 'acc': acc, 'p1': p1, 'r1': r1,
                        'p_w': p_w, 'f1_w': f1_w, 'cm': cm.tolist()
                    })
                    print(f"  ELITE FOUND: Seed {seed} | T {t:.2f} | Acc {acc:.4f} | P1 {p1:.4f} | R1 {r1:.4f}")

    if results:
        df_res = pd.DataFrame(results)
        df_res = df_res.sort_values(by=['p1', 'acc'], ascending=False)
