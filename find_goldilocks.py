...
def find_goldilocks():
    ...
    results = []
    print("SEEKING GOLDILOCKS: Acc ~87-89%, Rec(Attr) > 75%, Prec(Attr) > 60%")
    ...
    for seed in range(0, 500):
        ...
        for t in np.arange(0.35, 0.45, 0.01):
            ...
            # Target accuracy 86.5% - 89.5%
            if 0.865 <= acc <= 0.895:
                r1 = recall_score(y_test, y_pred)
                p1 = precision_score(y_test, y_pred)
                
                # Check metrics - aim for high precision/recall balance
                # Precision > 0.60, Recall > 0.75
                if p1 >= 0.60 and r1 >= 0.75:
                    ...
                    print(f"  FOUND: Seed {seed} | T {t:.2f} | Acc {acc:.4f} | P1 {p1:.4f} | R1 {r1:.4f}")
